...
def build_index():
    ...
    if sales_path.exists():
        s_df = pd.read_parquet(
            sales_path,
            columns=["listing_mode", "city", "locality", "price", "area", "society_name"],
        )
        dfs.append(s_df)
    if rentals_path.exists():
        r_df = pd.read_parquet(
            rentals_path,
            columns=["listing_mode", "city", "locality", "price", "area", "society_name"],
        )
        dfs.append(r_df)
    ...
    city_map = {"Gurgaon": "Gurugram", "Greater_Noida": "Greater Noida"}
    ...
    for (city, loc), group in grouped:
        ...
        for soc in societies.index:
            ...
            if not any(
                x in s_low
                for x in ["independent", "standalone", "other", "unknown", "nan", "block "]
            ):
