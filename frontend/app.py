...
with st.sidebar:
    ...
    sector_hints = SECTOR_HINTS.get(city, [])
    sector = st.text_input(
        "Sector / Locality",
        placeholder=f"e.g. {sector_hints[0] if sector_hints else 'Sector 50'}",
        help="Type the sector or locality name",
    )
    if sector_hints:
        with st.expander("Popular localities", expanded=False):
            st.caption(", ".join(sector_hints))
...
if predict_clicked:
    ...
    with st.spinner("Estimating price..."):
        try:
            ...
            st.markdown('<hr class="soft-divider">', unsafe_allow_html=True)
            st.caption(
                "**Disclaimer**: These estimates are generated by a machine learning model "
                "trained on historical data. Actual property prices may vary based on market "
                "conditions, specific location, property condition, and other factors. "
