...
os.environ["PYTHONPATH"] = str(Path(__file__).parent.absolute()) if "PATH" not in os.environ else str(Path(__file__).parent.absolute()) + os.pathsep + os.environ.get("PYTHONPATH", "")
...
def main():
    ...
    parser.add_argument("--train", action="store_true", help="Run the model training phase (Default)")
    parser.add_argument("--skip-train", action="store_true", help="Skip the model training phase")
    parser.add_argument("--quick-train", action="store_true", help="Run model training with only 1 trial")
