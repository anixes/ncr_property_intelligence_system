...
class RecommenderService:
    ...
    def find_similar(self, target: pd.Series, dataset: pd.DataFrame, 
                     top_k: int = 5, model_uncertainty: float = None) -> pd.DataFrame:
        """
        ...
        # 3. Cosine Similarity on normalized features
        available_features = [f for f in self.SIMILARITY_FEATURES if f in candidates.columns and f in target.index]
    ...
    def _score_deals(self, df: pd.DataFrame, model_uncertainty: float = None) -> pd.DataFrame:
        ...
        df = df.copy()
        undervaluation = (df['predicted_price'] - df['price']) / (df['predicted_price'] + self.EPSILON)
        
        uncertainty = model_uncertainty if model_uncertainty else df.get('prediction_uncertainty', 0.1)
    ...
    def explain_recommendation(self, target: pd.Series, comparable: pd.Series) -> Dict[str, Any]:
        ...
        if 'area' in target.index and 'area' in comparable.index:
            ...
            if abs(area_diff) > 5:
                ...
                explanations.append(f"Area is {abs(area_diff):.1f}% {direction} ({comparable['area']:.0f} vs {target['area']:.0f} sqft)")
