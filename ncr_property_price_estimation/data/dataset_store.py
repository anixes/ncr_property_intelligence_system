...
class DatasetStore:
    ...
    def save_stage(self, df: pd.DataFrame, stage: str, batch_id: str):
        ...
        if "listing_id" in df.columns:
            assert df["listing_id"].is_unique, (
                f"INTEGRITY ERROR: Duplicate listing_ids found in stage {stage}"
            )
        ...
        if "timestamp" in df.columns:
            ...
            assert df["timestamp"].is_monotonic_increasing, (
                "LEAKAGE ERROR: Timestamps are not monotonically increasing"
            )
    ...
    def load_latest(self, stage: str) -> pd.DataFrame:
        ...
        if current_checksum != latest_batch["checksum"]:
            raise ValueError(
                f"SECURITY ALERT: Checksum mismatch for {file_path}. Data may be tampered."
            )
