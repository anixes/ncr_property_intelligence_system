...
PROPERTY_SCHEMA = DataFrameSchema(
    {
        ...
        "price_per_sqft": Column(
            float,
            checks=[Check.in_range(10, 100000)],  # Support rent (~10-60) & sale (~1.5k-100k)
            nullable=False,
            coerce=True,
        ),
        # --- PHYSICAL SPECS ---
        ...
        "is_gated_community": Column(
            float, checks=[Check.isin([0, 1])], nullable=True, coerce=True
        ),
        "is_vastu_compliant": Column(
            float, checks=[Check.isin([0, 1])], nullable=True, coerce=True
        ),
        # Physical/Room Flags
...
def validate_dataframe(df):
    ...
    try:
        ...
        stats = {"total_rows": len(df), "valid_rows": len(clean_df), "dropped_rows": 0}
        ...
        print(
            f">> VALIDATION: Dropped {len(failure_indices)} rows ({len(failure_indices) / len(df) * 100:.1f}%)"
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"  Dropped:  {stats['dropped_rows']:,} rows ({stats['dropped_rows'] / stats['total_rows'] * 100:.1f}%)"
        )
        print("=" * 60)
