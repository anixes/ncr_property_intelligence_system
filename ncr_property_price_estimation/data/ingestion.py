"""
NCR Housing Scraper — [SAFE EXIT UPGRADE]
=============================================
...
class ResilientTracker:
    ...
    def save_checkpoint(self, is_rent: bool, city: str, bhk: int, page: int):
        ...
        if key not in self.checkpoint_data:
        ...
        self.checkpoint_path.write_text(
            json.dumps(self.checkpoint_data, indent=2), encoding="utf-8"
        )
    ...
    async def run(self, is_rent: bool = False):
        ...
        async with async_playwright() as p:
            ...
            for city in cities:
                ...
                if city not in cfg:
                    continue
                if r_city and NCR_CITIES.index(city) < NCR_CITIES.index(r_city):
                    continue

                for bhk in BHK_SLICES:
                    if r_city == city and bhk < r_bhk:
                        continue
                    ...
                    try:
                        ...
                        for p_idx in range(self.max_pages):
                            if r_city == city and r_bhk == bhk and p_idx < r_page:
                                continue
                            ...
                            if new_recs:
                                ...
                                (self.tracker.out_dir / fname).write_text(
                                    json.dumps({"data": new_recs}, indent=2), encoding="utf-8"
                                )
                                logger.info(f"    💾 Saved {len(new_recs)} NEW records.")
                            else:
                                consecutive_dupes += 1
                                logger.warning(
                                    f"    ⏩ Skipping page {p_idx + 1} (All duplicates). Dupes in a row: {consecutive_dupes}"
                                )
                            ...
                            if consecutive_dupes >= 3:
                                logger.info(
                                    f"  🛑 Inventory Exhausted for {city} {bhk}BHK. Moving to next slice."
                                )
                                break

                            if not await HousingExtractor.next_page(page):
                                break
