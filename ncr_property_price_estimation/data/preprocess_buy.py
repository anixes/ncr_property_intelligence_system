...
class BuyPreprocessor:
    ...
    def parse_area(self, text):
        ...
        match_sqft = re.search(
            r"(\d+)\s*(sq\.ft|sqft|sq-ft|square\s*feet|super\s*area)", text_clean
        )
        if match_sqft:
        ...
        if match_yards:
            ...
            if (
                val < 1000
            ):  # Heuristic: If it's 1200 it's already sqft, if it's 200 it's likely yards
                return val * 9
            return val
    ...
    def extract_nlp_features(self, df):
        ...
        df["ready_to_move"] = desc.str.contains(
            "available now|ready to move|immediate|possession status"
        ).astype(int)
        df["is_luxury"] = desc.str.contains("luxury|premium|ultra|high-end|posh").astype(int)
        df["is_gated_community"] = desc.str.contains(r"gated|security|24\*7").astype(int)
        ...
        def get_furnishing(txt):
            if re.search(r"fully|full[- ]?furnished", txt):
                return "Fully-Furnished"
            if re.search(r"semi[- ]?furnished", txt):
                return "Semi-Furnished"
            if re.search(r"unfurnished", txt):
                return "Unfurnished"
        ...
        df["is_park_facing"] = combined.str.contains(
            r"park facing|green facing|park view|green view"
        ).astype(int)
    ...
    def run(self):
        ...
        df["price"] = df.apply(
            lambda r: (
                self.parse_price(str(r["price_text"]))
                or self.parse_price(str(r["description_raw"]))
            ),
            axis=1,
        )
        df["area"] = df.apply(
            lambda r: (
                self.parse_area(str(r["area_text"])) or self.parse_area(str(r["description_raw"]))
            ),
            axis=1,
        )
        ...
        df["is_standalone"] = (
            df["title_raw"]
            .str.contains(r"Independent|Plot|House|Kothi|Villa|Bungalow", case=False, na=False)
            .astype(int)
        )
        ...
        def get_locality(row):
            ...
            m = re.search(
                r"\bin\s+(.+?)(?:,\s*" + re.escape(city_clean) + r"|\s*$)", title, re.IGNORECASE
            )
            if m:
        ...
        def refine_society_and_locality(row):
            ...
            parts = [
                p for p in parts if p.lower() not in [city.lower(), "delhi", "new delhi", "ncr"]
            ]
            ...
            if not society:
                for dev in DEVELOPERS:
                    m = re.search(
                        r"\b(" + re.escape(dev) + r"\s+[a-z0-9]+(?:\s+[a-z0-9]+)?)\b",
                        combined_text,
                        re.I,
                    )
                    if m:
            ...
            if not society:
                m = re.search(
                    r"\b([a-z0-9\-]+\s+(?:[a-z0-9\-]+\s+)?(?:Heights|Residency|Greens|Solitude|Estate|Villas|Floors|Gardens|Park|City|Township|Elite|Classic|Avenue|Plaza|Casa|Gems|Jewel|Pearl|Square|Enclave))\b",
                    combined_text,
                    re.I,
                )
            ...
            if not society:
                for p in parts:
                    if re.match(
                        r"^(block|pocket|phase|ward)\s+[R0-9A-Za-z-]+$", p.strip(), re.I
                    ) or p.strip().lower() in ["block", "pocket", "phase", "ward"]:
            ...
            for p in parts:
                p_lower = p.lower()
                is_generic = re.match(
                    r"^(block|pocket|phase|ward)\s+[R0-9A-Za-z-]+$", p_lower
                ) or p_lower in ["block", "pocket", "phase", "ward"]
        ...
        df.loc[df["prop_type"] == "Apartment", "society_name"] = df.loc[
            df["prop_type"] == "Apartment", "society_name"
        ].replace("Independent", "Standalone")
        df.loc[
            (df["is_standalone"] == 1) & (df["society_name"] == "Standalone"), "society_name"
        ] = "Independent"
        ...
        df["is_owner_listing"] = (
            df["agent_name"]
            .str.contains(r"Owner|Individual|None|A$|V$", case=False, na=False)
            .astype(int)
        )
        ...
        print(
            f"✅ SALE SUCCESS: Saved {len(clean_df)} records. Yield: {stats['valid_rows'] / stats['total_rows'] * 100:.1f}%"
        )
