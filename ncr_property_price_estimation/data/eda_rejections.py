...
def run_diagnostic_eda():
    print("🕵️ DIAGNOSTIC EDA: Investigating 16,000+ Rejections...")
    ...
    df["price"] = df.apply(
        lambda r: hp.parse_price(str(r["price_text"]), r["listing_mode"]), axis=1
    )
    df["area"] = df["area_text"].apply(hp.parse_area)
    df["bedrooms"] = df["bhk"].apply(
        lambda x: int(re.search(r"\d+", str(x)).group(0)) if re.search(r"\d+", str(x)) else 0
    )
