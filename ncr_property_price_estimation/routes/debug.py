...
def debug_model():
    ...
    return {k: {"type": str(type(v)), "meta": state.model_meta.get(k)} for k, v in state.models.items()}
...
def debug_fs():
    ...
    return {
        ...
        "data_contents": os.listdir(PROJ_ROOT / "data") if (PROJ_ROOT / "data").exists() else "Missing",
        "cwd": os.getcwd()
    }
